...
We now leverage the additional leakage that $\calF_{\LMKPM}$ outputs to $\tP_1$ to mount stronger attacks. We adapt Guo et al.'s search tree approach~\cite{USENIX:GHLWJL22}, designed for $\calF_{\PSICA}$, to recover the identifiers shared between the party inputs and achieve \emph{value intersection recovery} (Definition~\ref{def:VIR_goal}). 
