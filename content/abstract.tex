...
We first present a concrete attack on PSU that reconstructs the intersection using only two protocol invocations. Moreover, we show that any attack on PSI-Cardinality, such as that of Guo et al.\ (USENIX~2023), lifts to an attack on PSU-CA that recovers the intersection using only three additional queries. For the MKPM protocol, we distinguish its intended matching functionality from the additional protocol-specific leakage. We first give an attack against the intended functionality, and then show that exploiting MKPM’s additional leakage enables even stronger attacks, including partially reconstructing the other party’s records from a single protocol invocation.
