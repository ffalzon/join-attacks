...
In this work, without loss of generality and in line with prior work
\cite{USENIX:GHLWJL22,NDSS:JiaDuYan24,USENIX:FalTan25}, we assume that $P_1$ is
adversarial (denoted by $\tP_1$). The adversary follows the protocol
specification honestly, but may adaptively choose its inputs and repeatedly
invoke the functionality. Party $P_2$ is honest and holds a fixed set $Y$ of
elements, referred to as the \textbf{recovery set}, about which $P_1$ aims to
learn information. We note, however, that three of our attacks require only a
single query. Therefore, these three attacks also apply when $P_2$'s recovery
set is dynamic.

A \emph{target set} $T$ of values is also introduced, which is held by the
...
As discussed in Section~\ref{sec:leakage}, $\calF_{\LMKPM}$ is the extended
...
The first recovery goal is to recover the set of shared values. This captures
precisely the values that the adversary would like to identify in the recovery
set. Given record set $X$ (defined as in Section~\ref{sec:mkpmc_functionality}),
...
exists an index $i \in \N$ such that $\bx[i] = v$. We present the value recovery
goal formally as follows.
...
of $Y$. In particular, it reveals which shared values appear in the same record
and which values are repeated across multiple records---information that is not
captured by value intersection recovery.
...
\begin{definition}\label{def:max_recons}
  Let $T$ be the target set of records and $Y$ be the recovery set of
  records. An adversary $\adv$ achieves \textbf{$T$-reconstruction (TR)} of $Y$
  if it outputs a multiset $R_Y$ for which there exists an injective mapping
