...
We consider two recovery goals when invoking the \emph{leaky} MKPM functionality $\mathcal{F}_{\LMKPM}$. Exact record matching is often too strict of a recovery goal; our focus is thus on identifying common identifiers between the target and recovery sets. The first recovery goal against $\mathcal{F}_{\LMKPM}$ aims to recover the set of shared identifier. For any record set $X$, each record $\bx \in X$ is a vector of identifiers from the space $\Ispace{}$, i.e., $\bx \subseteq \Ispace{}$. We denote by $\Ispace{X}$ the set of identifiers present in the records of $X$, defined as
\[
