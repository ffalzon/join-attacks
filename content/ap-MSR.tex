...
\begin{lemma}
	...
	Let $X$ and $Y$ be two sets of records and let $(\hXone, \hYone)$ be the hidden shuffled records received by $\tP_1$.
	Moreover, let $f_1: \Ispace{} \to \Uspace$ be the hiding function sampled in $\calF_\LMKPM$ to produce $\hXone$ and $\hYone$.
	If $D$ is a dictionary mapping $f_1(v)$ to $v$ for all $v \in \Ispace[X]$,
	then $\dictInfer(D, \hYone)$ (Figure~\ref{fig:dict_infer}) produces a maximum reconstruction of $Y$ with respect to $X$.
\end{lemma}
\begin{proof}
	Let $R$ be the multiset produced by \dictInfer.
	% Recall from Section~\ref{sec:notation} that we consider different instantiations 
	% of the same record in $R$ as distinct elements.
	Note that for each $\hby \in \hYone$ exactly one reconstructed record $\br$
	is added to $R$.
	Let $d: R \to \hYone$ denote the function mapping these reconstructed records 
	to the corresponding $\hby$. Clearly, $d$ is a bijection.

	We now define an injective function $\varphi: R \to Y$ and prove that it satisfies the three conditions for Maximum Set Reconstruction (Definition \ref{def:max_recons}).
	For any record $\br \in R$ let $(d_1, \dots, d_k)=d(\br) \in \hYone$ and let
...
\begin{proposition}\label{thm:recenum_D}
	...
	Moreover, let $f_1$ be the identifier hiding function sampled during the evaluation of $\calF_\LMKPM$ in line~\ref{lin:recenum_eval} in Figure~\ref{fig:rec_enum_attack}. 
...
\begin{proof} \textbf{of Proposition~\ref{thm:recenum_D}} 
	...
	could produce two encodings that only contain one identifier, namely, $(v_0)^\ell$ and $(v_1)^\ell$
	...
	Hence, there is only one record in $X$ whose first $\ell$ identifiers all coincide,
	namely, $\bx_0$ with encoding $(v_0)^\ell$.
	Since $f_1$ is injective and the identifiers in records of $X$ are not shuffled, 
	...
	contains the identifiers $v_0$ and $v_1$.
	\begin{description}
	...
	\item[Case (b):] In this case, the vector of hidden identifiers corresponding to $\bs$ in the leakage, which we denote
...
\begin{proposition}
	...
	Moreover, let $f_1$ be the identifier hiding function sampled during the invocation of $\calF_\LMKPM$
...
\begin{proof} \textbf{of Proposition~\ref{thm:snake_D}}
	...
	We show that $D[f_1(v)] = v$ for all $v \in \Ispace[X]$.
	Since \snakeattack{} potentially adds fresh identifiers to records of $T$, but does not remove any, 
	...
	$\hbx_1$ is therefore the only record in $X$ whose $j^*$-th identifier is not contained in the last column, 
	...
	Moreover, all hidden identifiers in the $j^*$-th column of $\hXone$	are distinct. % $t'_{i+1}$ is the only record in $\setT'$
