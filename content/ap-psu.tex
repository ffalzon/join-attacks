	...
	\begin{pchstack}[boxed, center]
      ...
      \rule{0pt}{2em}%
		...
		\procedure[linenumbering]{$\PSICA\text{-}\textsf{Sim}(X)$}{
			\rule{0pt}{2em}%
      z \gets \PSUCA(X, Y) \\
			k \gets |X| + m - z \pccomment{Eqn.~\ref{eqn:union_intersect_cardinalities}} \\
			\pcreturn k
		}
	\end{pchstack}
...
\end{theorem}
\begin{proof}
  The theorem follows directly from the correctness of $\PSICAattack$ and Prop.~\ref{prop:PSUCA_comp_m} and Eq.~\ref{eqn:union_intersect_cardinalities}.
\end{proof}
