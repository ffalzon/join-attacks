\section{Additional Content for PSU-CA Attacks}
...
% \subsection{Pseudocode for PSU Attack}\label{ap:psu-attack-pseudocode}
...
\begin{proof}
  We provide a concrete construction of \bdv{} for any \adv{} in Fig.~\ref{fig:PSUCA_generic}.
  Correctness of \bdv{} follows directly from
  %Eq.~\ref{eqn:union_intersect_cardinalities} and 
  Prop.~\ref{prop:PSUCA_comp_m} and the fact that 
  we have $|X\cap Y| = |X| + |Y| - |X \cup Y|$ for any sets $X$ and $Y$.
  \bdv{} makes three PSU-CA evaluations on
...
the PSI-CA attack, translating union to intersection cardinalities. The attack is shown in
...
\begin{theorem}
  ...
  $\PSUCAattack^{\calF_{\PSUCA}(\cdot; Y)}(X)$ outputs $X \cap Y$.
\end{theorem}
\begin{proof}
  The theorem follows directly from the correctness of $\PSICAattack$ and Prop.~\ref{prop:PSUCA_comp_m} and the identity $|X \cap Y| = |X| + |Y| - |X\cup Y|$.
\end{proof}
...
p_{\PSICA}^*(k, |X|) &:= \frac{\max(k, |X| - k)}{|X|}\end{align*}
