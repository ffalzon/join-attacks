...
which may evaluate $\PSUCA(X', Y) = |X' \cap Y|$ for arbitrarily many $X'$ and a static recovery set $Y$. 
The goal of $A$ is to determine $X \cap Y$.
They present two search tree-based attacks against PSI-CA: 
(1) a generic attack assuming no prior knowledge about the recovery set and 
(2) a feature-aware attack, where the membership in the recovery set is correlated with a set of features that are known to the adversary.
...
\begin{figure}
	...
	{	\colorlet{positive}{LimeGreen}
		...
		as constructed for the generic intersection recovery attack from~\cite{USENIX:GHLWJL22}.
		We depict the attack behavior for a recovery set $Y = \{1, 2\}$. The PSI-CA functionality is invoked at nodes marked with {\protect\fullcirc{}}.
