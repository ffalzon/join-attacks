...
In this subsection, we evaluate our two search tree-based attacks: $\PSUCAattack$ against $\calF_{\PSUCA}$ and $\mkpsiattack$ against $\calF_{\MKPM}$.
...
\begin{figure}
  ...
  \subfloat[\PSUCAattack\label{fig:plot_ST_queries_over_mr:left}]{
    \begin{minipage}{0.23\textwidth}
      \centering
      \PSUQueryMRPlot{$|T| = 0.5 \cdot 10^4$}{}{}{measurements/PSU/PSUCA_queries_over_mr/V10000T5000.csv}{}
      \vspace{0.3em}
      \PSUQueryMRPlot{$|T| = 10^4$}{}{}{measurements/PSU/PSUCA_queries_over_mr/V10000T10000.csv}{}
      \vspace{0.3em}
      \PSUQueryMRPlot{$|T| = 1.5 \cdot 10^4$}{}{}{measurements/PSU/PSUCA_queries_over_mr/V10000T15000.csv}{}
    \end{minipage}
  }%
  ...
  \caption{Number of protocol queries required by the (a) $\PSUCAattack{}$ and (b) $\mkpsiattack$ attacks, plotted against the match rate $\rho$ for a fixed $|Y| = 10^4$.}
  \label{fig:plot_ST_queries_over_mr}
\end{figure}


\heading{Queries.}
Figure~\ref{fig:plot_ST_queries_over_n} plots the number of queries required by each attack as a function of the target set size.  
Figure~\ref{fig:plot_ST_queries_over_mr} plots the number of queries required by each attack as a function of $\rho$ and $\eta$, respectively. 
%We observe that both attacks make the most queries for values of $\rho$ and $\eta$ close to $0.5$. This matches our expectations: a balanced ratio reduces the likelihood of early path terminations in the search tree, thereby increasing the number of queries required to achieve full reconstruction.

Ignoring the trivial cases $\rho, \eta \in \{0,1\}$, the number of queries lies between $36\%$ and $73\%$ of the theoretical upper bounds, $n+1$ for \PSUCAattack{} and $n$ for \mkpsiattack{}. Values of $\rho$ and $\eta$ near $0.5$ incur the highest percentages: $36\%$ occurs for $\rho, \eta \in \{0.1, 0.9\}$ and $73\%$ for $\rho = \eta = 0.5$, and these percentages are consistent across target set sizes. Figure~\ref{fig:plot_ST_queries_over_n} also shows that the absolute number of queries grows linearly with the target set size, matching the linear theoretical bounds.

Both attacks make almost the same number of queries, despite \mkpsiattack{} having a stricter early-termination criterion. \ff{Is this stronger criterion explained anywherer??} For any experiment, their difference never exceeds $0.5\%$ of the theoretical upper bound, with \mkpsiattack{} slightly higher. The only exception is when $\rho$ or $\eta$ is $0$ or $1$: then \PSUCAattack{} always uses three queries, while \mkpsiattack{} uses only one, due to the two extra queries \PSUCAattack{} needs to determine $|Y|$.
