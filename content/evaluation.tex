...
\begin{figure}
	\centering
    \subfloat[$\PSUattack{}$\label{fig:plot_PSU_large_time:left}]{
    \input{plots/PSU_1M/time_mr.tex}
    }
    \hspace{0.1mm}
    \subfloat[$\PSUattack{}$\label{fig:plot_PSU_large_time:right}]{
...
Overall, the $\PSUattack$ attack (Section~\ref{sec:psu-attack}) is very efficient, as shown in Figure~\ref{fig:plot_PSU_large_time}.
On a large recovery set with $|Y| = 10^6$ elements, runtimes range from \SI{127}{\milli\second} to \SI{367}{\milli\second}. The measurements largely follow from Python's set-intersection implementation, which is linear in the size of the smaller operand.

Figure~\ref{fig:plot_PSU_large_time:left} shows the runtimes plotted against the intersection ratio $\rho$.
For large target sets, larger values of $\rho$ lead to a decrease in runtimes , since the increase of the shared elements between $T$ and $Y$ results in smaller sets $Z_i = X_i \cup Y$; this makes the computation of $Y = Z_1 \cap Z_2$ more efficient.
...
The runtime is dominated by computing the intersection $T \cap Y$,
which grows linearly in $|T|$ as long as $|T| \leq |Y|$ and stagnates after that.
The reason for the decreased runtimes for larger values of $\rho$ and target sets with $|T|>10^6$ is unclear, but preliminary tests suggest that the affected data sets exhibit a better cache locality.
