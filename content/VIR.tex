...
Our adapted attack starts by building a binary search tree $\calT$ over $T$ similar to Guo et al.'s attack~\cite{USENIX:GHLWJL22}. The root node corresponds to the entire target set $T$. Each node $v$ is labeled with a subset $v.\treeSet \subseteq T$. Its children $v_L \gets v.\treeLeftChild$ and $v_R \gets v.\treeRightChild$ form a disjoint partition of the parent:
...
\begin{theorem}\label{thm:VIR-attack-correctness}
  The $\mkpsiattack^{\calF_{\LMKPM}(\cdot;Y)}(T)$ attack (Figure~\ref{fig:MKPSIattack})
