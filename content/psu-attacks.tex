...
determine $X \setminus Y$. To address this, we use two additional observations.
...
The corresponding proofs can be found in
...
\ref{prop:PSU_comp_partial_difference} with three evluations of $\PSU$ to obtain
$X \cap Y$. However, since $X \cup Y = (X_1 \cup Y) \cup (X_2 \cup Y)$, the
adversary can compute the full union from the two partial unions, thus, saving
one evaluation.

Given two disjoint sets $X_1$ and $X_2$ that form a partition of the target set
$T$, i.e., $X_1 \cap X_2 = \emptyset$ and $X_1 \cup X_2 = T$, $\tP_1$ can
recover the intersection by querying $\calF_{\PSU}(X_1;Y)$ and
$\calF_{\PSU}(X_2;Y)$. The pseudocode for the attack \PSUattack{} is given in
Figure~\ref{fig:PSU_attack}
(Appendix~\ref{ap:psu-attack-pseudocode}). \tianxin{Naming the attack?}
