...
Recall that the corrupted party $\tP_1$ knows the target set $T$ and can invoke
$\calF_{\PSU}(;Y)$ on any set $X$. The adversary's goal is to recover
$T \cap Y$. Our attack is based on the following equation. For any two sets $X$
and $Y$ we have
\begin{equation}
  \label{eqn:PSU_basis}
...
Both $X \cup Y$ and $Y \setminus X$ can be easily determined. The former is the
output of evaluating $\calF_{PSU}(X;Y)$, and the latter can be computed as
$(X \cup Y) \setminus X = Y \setminus X$. The challenge lies in determining
$X \setminus Y$. To address this, we make two additional observations.
