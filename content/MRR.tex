...
Recall that the $\PSUCAattack{}$ attack (Section~\ref{sec:PSUCA_attack}) relies
...
$i \in \{1, 2\}$ respectively. \tianxin{Add.} Then we require the following
relation to hold:
\begin{equation}
...
\begin{figure}
  ...
  \caption{Example of a target set $T$ and a recovery set $Y$, such that
    ...
    $\calF_{\MKPM}(T;Y)$. Lines between records indicate potential matches based
    on matching values. Solid lines denote the matched records chosen by the
    matching logic. We see that $|\UID| \neq |\UID_1| + |\UID_2| - |Y|$ and, as
    such, $T$ is not $Y$-isolated.}
