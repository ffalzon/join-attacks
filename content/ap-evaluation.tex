...
In Figure~\ref{fig:plot_ST_queries_over_mr} we plot the number of protocol invocations required by the (a) $\PSUCAattack{}$ and (b) $\mkpsiattack$ attacks, plotted against the match rate $\rho$ for a fixed $|Y| = 10^4$.
...
The main interest was measuring how much of thw original recovery goal the attacks managed to achieve.
...
In contrast to Section~\ref{sec:st-queries}, the impact of various heuristics on attack performance is evident. Interestingly, the heuristic $p_{\PSICA}^+$, intended to maximize the recovery of $X \cap Y$, appears more efficient at increasing the recovery of $X \setminus Y$. Conversely, $p_{\PSICA}^-$ is more effective at optimizing the recovered intersection than the set difference. Additionally, $p_{\PSICA}^*$ underperforms across all metrics, including total membership inference, which it was meant to optimize.
