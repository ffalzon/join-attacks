...
The core of the protocol is the record-matching procedure $\match$ (the detailed pseudocode can be found in Appendix~\ref{ap:mkpc}), which takes shuffled inputs $X'$ and $Y'$. Each record in $X'$ (resp., $Y'$) is matched with at most one record in $Y'$ (resp., $X'$). For each identifier position $j$ in $X'$, $\match$ scans all $\by_k \in y'$ and matches $\by_k$ to the first $\bx_i \in X'$ whose $j$-th identifier appears in $\by_k$:
\[
\exists \ell \text{ such that } \by_k[\ell] = \bx_i[j],
\]
