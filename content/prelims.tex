...
$\Ispace$ denote the universe of record values and let $n_i, m_k $ denote the
record length in set $X$ and $Y$, respectively, for all $i \in [n]$ and all
$k \in [m]$.
...
The functionality takes $X$ and $Y$ as inputs and then applies a random
permutation. The records of $X$ are permuted while preserving the order of
identifiers within each record, whereas in $Y$ both the records and the
identifiers within each record are permuted. We denote the resulting collections
