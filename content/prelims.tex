...
\begin{figure*}[t]
    ...
    \caption{
    Example inputs (a) and outputs (b) of the $\MKPM$ protocol. In (a) we depict a one-to-many match (red edges): Party $P_1$’s User 1 matches Party $P_2$’s User 1 on email and Party $P_2$’s User 2 on phone. The conflict is resolved randomly, yielding a one-to-one mapping in which $P_1$’s User 1 and $P_2$’s User 1 share the same UID, 2093, while $P_2$’s User 2 is assigned UID 9349. Party $P_1$’s User 2 matches only $P_2$’s User 3 on email (green edge) and thus are assigned UID 3042. Both parties learn their local UID mappings and the universe of UIDs.
