...
\begin{figure*}[t]
    \centering
    \subfloat[\label{fig:match-input}The inputs from Parties $C$ (left) and $P$ (right).]%
    {
    \includegraphics[clip, trim=0cm -1.1cm 0cm 0cm,width=1.34\columnwidth]{figures/match-input.pdf}} 
    \quad
    \subfloat[\label{fig:match-output}The outputs to Parties $C$ (left) and $P$ (right).]%
    {
...
\begin{figure}
    ...
    \begin{minipage}[t]{0.98\columnwidth}
		...
		\textbf{Functionality} $\calF_{\MKPM}(\calC, \calP)$ \dbox{$\calF_{\LMKPM}(\calC, \calP)$}~: 
        %%%
        \begin{enumerate}
        ...
        \begin{enumerate}
				...
				\item  $\calC'' := \{c''_i \setdsc i \in [n]\}$, where $c''_i := c_{\pi(i)}$.
	...
	\caption{The Multi-Key Private Matching functionality $\calF_{\MKPM}(\calC, \calP)$. The additional steps of the leakage-aware functionality $\calF_{\LMKPM}(\calC, \calP)$ are shown in \protect\dbox{dashed boxes}.
...
The leakage to party $C$ consists of a renamed copy of the shuffled sets $\calC'$ and 
$\calP'$. Concretely, we sample an injective random function $f_C:\Ispace{}\to\calG$, referred to as a hiding function, and apply it to all identifiers in $\calC'$ and 
$\calP'$. Party $C$ observes the resulting hidden sets.

Equivalently, the leakage to $C$ can be viewed as a bipartite graph $G = (V_C \cup V_P, E)$, where $V_C$ corresponds to the records in $\calC'$ and
$V_P$ corresponds to the records in $\calP'$. An edge $(c_i, p_k) \in E$ exists if and only if the records
$c_i \in \calC'$ and $p_k \in \calP'$ share at least one identifier. Further note that, because the identifier order within records of $\calC'$ is preserved, the matching process induces a priority ordering over edges, determined by the position of shared identifiers within records of $\calC'$.


Although the procedure $\match$ computes only a partial matching in $G$, such that each vertex is matched to at most one
vertex in the other vertex set, party $C$ still learns the full graph $G$.

The leakage observed by $P$ consists only of a hidden copy of $\calC$. The records of $\calC$ with fresh randomness, yielding a set $\calC''$. Then, a new hiding function $f_P : \Ispace \to \calG$ is sampled and used to hide the identifiers in $\calC''$.
$P$ observes the hidden and re-shuffled set $\calC''$.
...
\begin{definition}[Maximum Set Reconstruction]\label{def:max_recons}
	...
	\begin{enumerate}
		\item For all $y^* \in R_Y$ and all $\id \in y^*$ we have $\id \in \varphi(y^*)$.
		\item For all $y^* \in R_Y$ 
		and all $\id \in\varphi(y^*)$ we have if $\id \in \Ispace{T}$ 
		then $\id \in y^*$.
		\item For all $y \in Y$, if there is some $\id \in y$ 
		with $\id \in \Ispace{T}$ then $y \in \varphi\left(R_Y\right)$. 
	\end{enumerate} 
\end{definition}

Injectivity of $\varphi$ ensures that each $y^* \in R_Y$ corresponds to exactly one $y \in Y$.
