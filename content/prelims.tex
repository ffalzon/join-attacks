...
\begin{figure}
    ...
    \begin{minipage}[t]{0.98\columnwidth}
        ...
        \begin{enumerate}
				...
				\item  $f_X, f_Y \sample \InjFuncs[\Ispace,\; \calG]$.
				\item  $\leak_\calC^C := \{(f_X(\bx'_i[1]), \dots, f_X(\bx'_i[n_i])) \setdsc i \in [n]\}$.
				\item  $\leak_\calP^C := \{(f_X(\by'_i[1]), \dots, f_X(\by'_i[m_i])) \setdsc i \in [m]\}$.
				\item  $\pi \sample \Perms[[n]]$.
				\item  $X'' := \{\bx''_i \setdsc i \in [n]\}$, where $\bx''_i := \bx_{\pi(i)}$. \ff{Is it correct or = $\bx'$}
				\item  $\leak_\calC^P := \{(f_Y(\bx''_i[1]), \dots, f_Y(\bx''_i[n_i])) \setdsc i \in [n]\}$.
			\end{enumerate}
		\end{adjustbox}
			\begin{enumerate}
			\addtocounter{enumi}{13}
			\item Output:\hfill \begin{itemize}
					\item Send $\UID, \MX$ \dbox{$,\left(\leak_\calC^C, \leak_\calP^C\right)$} to $P_1$.
					\item Send $\UID, \MY$ \dbox{$,\leak_\calC^P$} to $P_2$.
			\end{itemize}
		\end{enumerate}
    \end{minipage}
    }
...
The leakage observed by $P_2$ consists only of a hidden copy of $X$. The records of $X$ with fresh randomness, yielding a set $X''$. Then, a new hiding function $f_Y : \Ispace \to \calG$ is sampled and used to hide the identifiers in $X''$.
