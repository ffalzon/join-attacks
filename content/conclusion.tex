...
\heading{Modified Functionalities.} Since these functionalities inherently
...
% De Cristofaro and Tsudik~\cite{FC:DeCTsu10} introduced Authorized PSI in which
% each element in the client set must be authorized (signed) by some recognized
% and mutually trusted authority.  However, when inputs change (e.g., a user is
% added to a database), a fresh commitment must be computed, so inputs must
% change infrequently or remain static.  The authors highlight applications that
% tolerate this limitation, such as Apple's CSAM detection
% system~\cite{AppleCSAM}.
...
% This raises concerns about users' privacy expectations, especially since
% attacks that leverage adversarially chosen inputs are often left unaddressed.
