...
Recent works have sought to extend two-party joins to multiple parties~\cite{CCS:MohRinRos20,PoPETS:LehMouSid26} and the delegated compute setting~\cite{PoPETS:MMTSBC24}; such variants typically rely on a non-collusion assumptions between parties. At the same time, we observe growing adoption of MPC technologies by large organizations (e.g., Google's Private Join and Compute~\cite{EuroSP:IKNPSS20}). 
