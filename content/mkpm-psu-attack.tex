...
In this section, we demonstrate how the $\PSUCAattack{}$ attack can be applied to the functionality of Meta's PrivateID protocol, $\calF_{\SKPM}$, and (with some additional assumptions) to the $\calF_{\MKPM}$ functionality formalized in Figure~\ref{fig:SKPM_definition}.

\subsection{Applying the PSU-CA Attack to PrivateID}
Recall the functionality $\calF_{\SKPM}$ defined in Figure~\ref{fig:SKPM_definition}. Since each element $z \in Z = X \cup Y$ is mapped to its unique identifier under an injective function $f$, the relationship $|\UID| = |X \cup Y|$ holds for any sets $X$ and $Y$. Consequently, we can directly apply $\PSUCAattack{}$ to $\calF_{\SKPM}$ to recover $X \cap Y$ in at most $|X| + 1$ protocol invocations. Guo et al.~\cite{USENIX:GHLWJL22} examined PrivateID in the context of their intersection recovery attack on $\PSICA{}$, where they recover $X \cap Y$ in at most $|X|$ protocol invocations. However, their approach relies on the fact that PrivateID leaks $|X \cap Y|$ during protocol execution, whereas our attack depends solely on the functionality output.

\subsection{Applying the PSU-CA Attack to MKPM}
Meta's MK-PrivateID protocol~\cite{MPMC} implements the Multi-Key Private Matching (MKPM) functionality we defined in \ref{sec:prel_leakage}.
...
This corresponds the \emph{Matchable Record Recovery} goal we formalized in Figure~\ref{sec:MRR_goal}.
Recall that the attacker is given a set of records $Y$, the target set, and can evaluate $\calF_{\MKPM}(X, Y)$
for arbitrarily many inputs $X$, while the victim set $Y$ remains static.
The adversary's goal is to determine the set $T \sqcap Y  = 
\{\bt \in T \setdsc \exists v \in 
Y \; \exists j, \ell \in \NN : t[j] = v[\ell]\}$.
