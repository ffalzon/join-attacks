...
show a straghtforward set-based attack against PSU. The attack only requires two
...
%% Our first attack targets the PSU functionality. Through show that when the
%% protocol is invoked on two disjoint subsets forming a partition of $T$ (e.g.,
%% $T = X_1 \cup X_2$), the adversary can fully recover the intersection $T \cap
%% Y$. \emph{This result significantly constrains how PSU can be safely used,
%% both as a standalone protocol and as a component within more complex
%% systems.}

%% Next, we consider PSU-CA. We demonstrate that although PSU-CA reveals only
%% the cardinality of the set union, we can nevertheless recover the
%% intersection $T \cap Y$ through a sequence of adaptively chosen queries. We
%% establish a direct connection to PSI-CA, showing that any attack against
%% PSI-CA (e.g.,~\cite{USENIX:GHLWJL22,NDSS:JiaDuYan24,USENIX:FalTan25}) can be
%% transformed into an attack against PSU-CA using only three additional
%% queries. \emph{This attack demonstrates how vulnerabilities in one
%% functionality can be leveraged to attack a broader class of functionalities.}
