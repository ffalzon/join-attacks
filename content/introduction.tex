...
Functionality leakage is an inherent characteristic of many systems that allow two or more parties to compute over private inputs. Such systems include secure multi-party computation (MPC) and zero-knowledge (ZK) proofs~\cite{STOC:GolMicRac85}. These primitives are no longer purely theoretical: MPC and ZK functionalities have been deployed in practice, for example in Google’s Private Join and Compute protocol for private ad-conversion measurement~\cite{googlePJC} and in Zcash, which uses ZK proofs to enable private transactions without revealing the sender, receiver, or transaction amount~\cite{zCash}.

It is well understood that, given a known function and access to input–output pairs, an adversary can infer information about the inputs. The deployment of these systems, however, makes it pressing to understand \emph{what} information can be inferred by parties participating in such protocols, and under which usage patterns and threat models this leakage becomes practically significant.


Among the most widely studied and deployed MPC protocols are set-operation functionalities, such as private set intersection (PSI), which enable parties to compute an intersection of their datasets without leaking the . PSI has been extensively explored in the literature, both from a protocol-design and an attack-analysis perspective (see Section~\ref{sec:related-work}). In this work, we instead focus on private set union (PSU), where two or more parties, each holding a set, jointly compute the union of their sets without revealing anything beyond the result~\cite{USENIX:DZBC25,USENIX:TBZCC25,USENIX:JSZG24,USENIX:ZCLZL23,USENIX:JSZDG22,CCS:GNBT25,CCS:ZCLPHW24,CCS:TCLZ23,ASIACCS:CSSW25,ASIACCS:BlaAgu12,AC:LiuGao23,AC:KRTW19,PoPETS:GaoNguTri24,ACNS:Frikken07,C:KisSon05}.
...
including repeated values in the inputs and certain syntactic
inconsistencies. Moreover, the protocol reveals additional information beyond
simply the matched values: party $P_1$ learns a renamed and shuffled copy of
...
\tianxin{Threat models: more specific.}
