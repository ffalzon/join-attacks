...
Today, these primitives are increasingly deployed in practice. For example, Google’s Private Join and Compute protocol uses MPC for private ad-conversion measurement~\cite{googlePJC}, and Coinbase's MPC wallet employs MPC to split a client's private key into two shares---one held by the client and the other held by Coinbase~\cite{Coinbase}. At the same time, it is well known that given a function and access to input-output pairs, an adversary can infer additional information about the inputs. We emphasize that this information leakage is intrinsic to the functionality itself and not a failure of the protocol to meet security according to its defined security. The growing adoption of such MPC protocols makes it critical to understand what information can be inferred by participants and under which usage patterns and threat models such leakage becomes practically significant. 

Among the most widely studied and deployed MPC protocols are set-operation functionalities, such as private set intersection (PSI), which allow parties to privately compute the intersection of their datasets. PSI-like functionalities have been extensively explored in both protocol design and attack analysis (see Section~\ref{sec:related-work}). However, prior cryptanalytic work that considers adversaries who honestly execute the protocol and seek to infer information from the output is narrow in scope, focusing almost exclusively on PSI-like protocols such as PSI-Cardinality~\cite{USENIX:GHLWJL22, NDSS:JiaDuYan24} and Private Join and Compute~\cite{USENIX:FalTan25} (which computes an inner product over values whose keys lie in the intersection). This leaves a broad range of other functionalities largely unstudied in this adversarial setting.
