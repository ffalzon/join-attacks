...
If this enumeration can be recovered from the protocol leakage, we can construct a dictionary $\dictInfer$ from hidden to plaintext identifiers and then use $\dictInfer$ (see \ref{fig:dict_infer}) to output a maximum reconstruction of the victim's input $Y$.
...
We now consider the leakage that results from invoking MK-PrivateID on $X$;
...
\begin{figure*}
    ...
    \rule{0pt}{2em}%
		\text{Choose three distinct $\id_0, \id_1, \id_2\gets\calI$} \\
		\text{Pick some ordering $T = (\bt_1, \dots, \bt_n)$} \\
	     \ell \gets \lceil \log_2 n \rceil \\
		\pcfor i = 0, \dots, n -1 \pcdo\\
		\t	\bs \gets \IdxToSeq(i, \ell, \id_0, \id_1, \id_2) \\
		\t	\bx_i \gets \bs \| \bt_i \label{lin:recenum_t_prime}\\
        \t X \gets (\bx_0,\dots, \bx_{n-1}) \\
		(\UID, \MC, (\hXone, \hYone)) \sample \calF_{\LMKPM}(X, Y) \label{lin:recenum_evaluation} \pcskipln\\
		\cmmnt{Find value corresponding to $\id_0$.} \\
		\text{Find $\bx^*_0 \in\hXone$ s.t. $\bx^*_0[1] = \dots = \bx_0^*[\ell]$} \label{lin:recenum_t0}\\
		e_0 \gets t^*_0[1] \label{lin:recenum_e0} \label{lin:assign_e0} \pcskipln\\
		\cmmnt{Construct substitution map $D$. }\\
		\text{Initialize empty dictionary $D$.} \\
		\pcfor \bx^* \in \hXone \pcdo \label{lin:recenum_D_start} \\
		\t	i \gets \SeqToIdx(\bx^*, \ell, e_0) \\
		\t	\pcfor j = 1, \dots, |\bx^*| \pcdo \\
		\t	\t	D\left[\bx^*[j]\right] \gets \bx_i[j] \label{lin:recenum_D_end}\\
		R_Y \gets \dictInfer(\hYone, D) \\
		\pcreturn R_Y
	}
	\end{pcvstack}
	\pchspace
    ...
    \rule{0pt}{2em}%
		...
		\t	\t 	s \gets s \| \id_0 \\
		\t	\pcelse \\
		\t	\t	s \gets s \| \id_1 \label{lin:idx_to_seq_bin_encode_end} \\
		\pcreturn s
	}
	\procedure[linenumbering, lnstart=28]{$\SeqToIdx(\bx^*, \ell, e_0)$}{
    \rule{0pt}{2em}%
		\pcif \forall j \in [\ell], \bx^*[j] \neq e_0 \pcthen \label{lin:seq_to_idx_tiebreak} \pcskipln\\
	...
	Note that $Y$ is part of the oracle and not known to the adversary. $\dictInfer$ is defined in Figure~\ref{fig:toy_attack}.}\label{fig:rec_enum_attack}\label{fig:idx_to_seq}\label{fig:seq_to_idx}
\end{figure*}
...
\begin{theorem}
	Let $T$ be the adversary's target set and $Y$ be the victim's tables of records.
$\recenumattack^{\calF_\LMKPM(\cdot, Y)}(T)$ (Figure~\ref{fig:rec_enum_attack}) produces a maximum reconstruction of $Y$ w.r.t. $T$ (Def.~\ref{def:max_recons}).
\end{theorem}
