\section{More attacks against $\calF_{\LMKPM}$}\label{sec:lmkpm-attacks}

In this section, we describe three powerful attacks that exploit the ``leaky'' functionality $\calF_{\LMKPM}$. 
The authors of MK-PrivateID \cite{MKPMC} acknowledge the existence of their protocol’s leakage but deem it acceptable as an aggregate metric that would only cause concern across multiple executions. We contradict this by presenting three attacks that achieve \emph{maximum set reconstruction} (Definition~\ref{def:max_recons}) with only a single protocol invocation.
