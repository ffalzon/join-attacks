...
In this section, we describe three powerful attacks that exploit the ``leaky'' functionality $\calF_{\LMKPM}$. The authors of MK-PrivateID \cite{MKPMC} acknowledge the existence of their protocol's leakage but deem it acceptable, as it is an aggregate metric that would only cause concern when involved in multiple executions. 
...
All three attacks involve preprocessing the target set $T$ in such a way that its modified records are uniquely identifiable from the protocol's output.
