...
While the record enumeration attack improves over the baseline in terms of detectability, it still requires an input whose record lengths grow with the target set size. We address this limitation by observing that it suffices to impose an order on the records without explicitly assigning a unique index to each one. This approach extends each record by only two identifiers, so every record in the modified input set $X$ is larger than its counterpart in $T$ by only a constant additive factor.

To start, that there exists a column $j^*$ in $T$ in which every identifier is unique, i.e., $|\{\bt[j^*] \setdsc \bt \in T\}| = |T|$.
This is not unreasonable, since many identifiers such as email addresses are usually unique across user data.
If not such column exists, the adversary can create one by sampling $n$ distinct identifiers
...
\heading{Time and query complexity.} 
\snakeencodeUnique{} has an asymptotic average-case time complexity of \bigO{n} for $n = |T|$,
