...
%DIF DEL pre_revision/main.tex   Thu May 14 19:43:52 2026
...
\DIFadd{In Guo et al.’s attack, the adversary has a target set \(X\) and can query the PSI-CA functionality \(\calF_{\PSICA}(\cdot ;Y)\) on adaptively chosen subsets \(X' \subseteq X\), where \(Y\) is a fixed recovery set. The goal is to recover \(X \cap Y\). The key observation is that some PSI-CA outputs immediately reveal membership information: (1) if \(|X' \cap Y| = 0\), then no element of \(X'\) lies in \(Y\) and (2) if \(|X' \cap Y| = |X'|\), then every element of \(X'\) lies in \(Y\).
}
...
\begin{figure}
  ...
  {	\colorlet{positive}{LimeGreen}
    ...
    \end{tikzpicture}
    \caption{
      \DIFaddFL{Binary search tree for \(X=\{1,2,3,4,5,6\}\) in the generic intersection-recovery attack of~\mbox{%DIFAUXCMD
\cite{USENIX:GHLWJL22}}\hskip0pt%DIFAUXCMD
